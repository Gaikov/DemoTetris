...
// nsFigureBase::Init: 
//---------------------------------------------------------
bool nsFigureBase::Init()
{
	if ( !g_figBase )
	{
		g_figBase = new nsFigureBase;
		if ( !g_figBase->m_list.size() )
			return false;
	}
	return true;
}
...
//---------------------------------------------------------
const figure_t* nsFigureBase::GetFigure( int idx )
{
	if ( idx < 0 || idx >= m_list.size() ) return 0;
	return &m_list[idx];
}
...
//---------------------------------------------------------
bool nsFigureBase::ParseFigure( script_state_t *ss, figure_t &fv )
{
	...
	do 
	{
		...
		if ( row )
		{
			...
			if ( maxCols < strlen( row ) )
				maxCols = strlen( row );

			fv.rows[currRow].resize( maxCols, ' ' );
			memcpy( &fv.rows[currRow][0], row, strlen( row ) );
		}
	}
