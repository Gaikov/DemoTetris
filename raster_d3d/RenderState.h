...
public:
	...
	//init render
	virtual bool					ConfigDlg( uint hWnd, const char *localPath, const char *locaPostfix ); //диалог конфигурации, вызывается приложением только перед началом использования
	virtual bool					Init( uint hWnd );	//инициализация, вызывается приложением перед началом использования
	virtual void					Release(); //освобождение, вызывается приложением после использования 
	virtual int						GetModeList( int **modes );
	virtual const rasterConfig_t*	GetCurrentConfig();

	//текущий цвет отображения примитивов (не нужно)
	...
	virtual void			DrawPlaneSprite( const nsMatrix &view, nsVec3& p1, nsVec3& p2, float width, uint wrap_count );
